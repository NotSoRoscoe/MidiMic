...
void loop() {
  ...
  if (threshReached) {
   ...
   // Serial.println(getMidiVal(freq));
    //
  }

  if (onNoteStillOn && !threshReached) {
    usbMIDI.sendNoteOff(onNote, vel, CHANNEL);
  }
  if (!threshReached) {
    usbMIDI.sendNoteOff(onNote, vel, CHANNEL);
    onNoteStillOn = false;
  ...
  if (threshReached) {
    onNote = getMidiVal(freq);
    usbMIDI.sendNoteOn(onNote, vel, CHANNEL);
    onNoteStillOn = true;
  }
